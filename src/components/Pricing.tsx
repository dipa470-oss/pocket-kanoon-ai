...
import { PLAN_PRICING, type PlanId } from "@/lib/pricing";
...
export function Pricing() {
  ...
  const { startCheckout, busy } = useRazorpayCheckout(refresh);
            ...
            return (
              ...
              >
                ...
                <div className="flex items-baseline gap-2 mb-6">
                  <span className="font-display text-5xl">{p.priceDisplay}</span>
                  <span className="text-sm text-muted-foreground">{p.period}</span>
                </div>
                <button
                  disabled={busy || isCurrent}
                  onClick={() => handlePlan(p.id)}
                  className={`w-full py-3 rounded-md font-medium transition-all disabled:opacity-60 ${
                    "highlight" in p && p.highlight
